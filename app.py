...
model = tf.keras.models.load_model(str(BASE_DIR / 'model.h5'))

## Load the encoders and scaler
try:
    with open(BASE_DIR / 'label_encoder_gender.pk1', 'rb') as file:
        label_encoder_gender = pickle.load(file)
except Exception:
    data = pd.read_csv(BASE_DIR / 'Churn_Modelling.csv')
    label_encoder_gender = LabelEncoder()
    label_encoder_gender.fit(data['Gender'])

with open(BASE_DIR / 'onehot_encoder_geo.pk1', 'rb') as file:
    onehot_encoder_geo = pickle.load(file)

with open(BASE_DIR / 'scaler.pk1', 'rb') as file:
